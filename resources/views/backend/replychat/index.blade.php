<link rel="stylesheet" href="{{ asset('/backend/assets/css/chatbox.css') }}?v={{ time() }}">
...
<body class="bg-gray-100">
    ...
    <script>
        ...
        async function sendMessage() {
            ...
            try {
                const url = `${window.location.origin}/admin/chat/reply`; // dùng origin hiện tại, tránh base href khác
                const res = await fetch(url, {
                    method: 'POST',
                    credentials: 'same-origin', // gửi cookie/session để middleware auth nhận
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json', // yêu cầu JSON, tránh redirect HTML
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                    },
                    body: JSON.stringify({
                        token: currentToken,
        ...
        async function loadMessages(token) {
            ...
            try {
                ...
                const res = await fetch(`${baseUrl}/admin/chat/load?token=${token}`);
